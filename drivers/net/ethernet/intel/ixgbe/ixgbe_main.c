...
static void ixgbe_xdp_rs_and_bump(void *xdpsq, bool sent, bool flush)
{
	struct ixgbe_ring *xdp_ring = xdpsq;
...
LIBETH_XDP_DEFINE_FINALIZE(static ixgbe_xdp_finalize_xdp_napi,
			   ixgbe_xdp_flush_tx, ixgbe_xdp_rs_and_bump);
				   ...
				   const int budget)
{
	...
	while (likely(total_rx_packets < budget)) {
		...
		if (unlikely(rx_ring->hdr_pp)) {
			...
			if (!xdp->data) {
				hdr_size = ixgbe_rx_hsplit_wa(hdr_buff,
							     rx_buffer,
							     size);
				size -= hdr_size ? : size;
			}
		...
		if (xdp_tx_bulk.prog &&
		    !ixgbe_xdp_run_prog(xdp, &xdp_tx_bulk)) {
			xdp->data = NULL;
			total_rx_packets++;
...
static int ixgbe_rx_create_pp(struct ixgbe_ring *rx_ring)
{
	 ...
	 */
	frame_size =
		IXGBE_RX_SRRCTL_BUF_SIZE(READ_ONCE(rx_ring->netdev->mtu));
	fq.hsplit = (adapter->flags2 & IXGBE_FLAG2_HSPLIT) &&
