...
bool ixgbevf_xsk_alloc_rx_bufs(struct ixgbevf_ring *rx_ring, u32 num)
{
	...
	IXGBEVF_RX_DESC(rx_ring, fq.ntu)->wb.upper.length = 0;
...
LIBETH_SQE_CHECK_PRIV(struct ixgbevf_zc_sqe_priv);
			     ...
			     struct ixgbevf_ring *rx_ring, int budget)
{
	...
	while (likely(total_rx_packets < budget)) {
		...
		if (ixgbevf_is_non_eop(rx_ring, rx_desc) || unlikely(!head_xdp))
			continue;
